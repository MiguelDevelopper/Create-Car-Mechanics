...
public class CVW_SoundEvents {

	public static final Map<ResourceLocation, SoundEntry> ALL = new HashMap<>();

	public static final SoundEntry

		VEHICLE_PARTS_MAKER_ACTIVATE = create("vehicle_parts_maker_activate").subtitle("vehicle parts making")
			.category(SoundSource.BLOCKS)
			.build();

	private static SoundEntryBuilder create(String name) {
		return create(CVW_main.resourceLocation(name));
	}
	...
	public static void prepare() {
		for (SoundEntry entry : ALL.values())
			entry.prepare();
        CVW_main.LOGGER.info("Preparing Sound Events for " + CVW_main.ID);
	}
...
//	}
	...
	private static class SoundEntryProvider implements DataProvider {
		...
		public String getName() {
			return "Create vehicular works Custom Sounds";
		}

		public CompletableFuture<?> generate(Path path, CachedOutput cache) {
			path = path.resolve("assets/create_vehicular_works");
	...
	public static class SoundEntryBuilder {
		...
		public SoundEntryBuilder addVariant(String name) {
			return addVariant(CVW_main.resourceLocation(name));
		}
