...
public class SequencedAssemblyPartsMakingRecipeSerializer extends ProcessingRecipeSerializer<PartsMakingRecipe> {
    ...
    public SequencedAssemblyPartsMakingRecipeSerializer(ProcessingRecipeBuilder.ProcessingRecipeFactory<PartsMakingRecipe> factory) {
        super(factory);
    }

    @Override
    protected void writeToJson(JsonObject json, PartsMakingRecipe recipe) {
    ...
    @Override
    protected PartsMakingRecipe readFromJson(ResourceLocation recipeId, JsonObject json) {
        ItemStack output = readOutput(json);
        Ingredient input = readIngredient(json);
        return new PartsMakingRecipe(input, output, recipeId);
    }

    @Override
    protected void writeToBuffer(FriendlyByteBuf buffer, PartsMakingRecipe recipe) {
        buffer.writeItem(recipe.output);
        recipe.ingredient.toNetwork(buffer);
    }

    @Override
    protected PartsMakingRecipe readFromBuffer(ResourceLocation recipeId, FriendlyByteBuf buffer) {
